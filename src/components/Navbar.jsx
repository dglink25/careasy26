// src/components/Navbar.jsx — icônes React Icons à la place des emojis
...
// ── Cache localStorage ────────────────────────────────────────────────────────
const PROVIDER_CACHE_KEY = 'careasy_is_provider';
...
function getCachedProvider(userId) {
  try {
    ...
    if (parsed.userId === userId) return parsed.isProvider;
  } catch {}
  return null;
}

function setCachedProvider(userId, isProvider) {
  try {
    localStorage.setItem(PROVIDER_CACHE_KEY, JSON.stringify({ userId, isProvider }));
  } catch {}
}

// ── Icônes de notification (vraies icônes, plus d'emojis) ────────────────────
const NOTIF_ICONS = {
  message:                { Icon: FiMessageSquare, color: '#3b82f6', bg: '#eff6ff' },
  rdv_pending:            { Icon: FiClock,         color: '#f59e0b', bg: '#fffbeb' },
  rdv_confirmed:          { Icon: FiCheckCircle,   color: '#10b981', bg: '#ecfdf5' },
  rdv_cancelled:          { Icon: FiXCircle,       color: '#ef4444', bg: '#fef2f2' },
  rdv_completed:          { Icon: FiAward,         color: '#8b5cf6', bg: '#f5f3ff' },
  entreprise_approved:    { Icon: FiCheckCircle,   color: '#10b981', bg: '#ecfdf5' },
  entreprise_rejected:    { Icon: FiAlertTriangle, color: '#ef4444', bg: '#fef2f2' },
  new_entreprise_pending: { Icon: FiBell,          color: '#f59e0b', bg: '#fffbeb' },
  trial_started:          { Icon: FiZap,           color: '#10b981', bg: '#ecfdf5' },
  default:                { Icon: FiBell,          color: '#6b7280', bg: '#f9fafb' },
};
        ...
        {unreadCount > 0 && (
          <button
            ...
            style={{
              ...
              color: 'var(--brand-primary)', fontSize: '0.8rem', fontWeight: 600,
      ...
      <div style={{ overflowY: 'auto', flex: 1, maxHeight: 440 }}>
        {loadingNotifs ? (
          ...
          <div style={{ padding: '3rem', textAlign: 'center' }}>
            {/* ✅ Vraie icône à la place de l'emoji cloche */}
            <div style={{
          ...
          dbNotifs.map((notif) => {
            ...
            const { Icon: NotifIcon, color, bg } = meta;
            ...
            return (
              <div
                ...
                style={{
                  ...
                  backgroundColor: isRead ? 'transparent' : `${color}0d`,
                  borderBottom: '1px solid var(--border-color)',
                  transition: 'background .2s', position: 'relative',
                }}
                onMouseEnter={e => { e.currentTarget.style.backgroundColor = 'var(--bg-secondary)'; }}
                onMouseLeave={e => { e.currentTarget.style.backgroundColor = isRead ? 'transparent' : `${color}0d`; }}
              >
                {/* ✅ Pastille non-lu */}
                {!isRead && (
                  <div style={{
                ...
                {/* ✅ Icône React Icons dans un cercle coloré */}
                <div style={{
                ...
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '0.3rem', fontSize: '0.9rem',
      ...
      {dbNotifs.length > 0 && (
        <div style={{
          ...
          <button
            ...
            style={{
              background: 'none', border: 'none', cursor: 'pointer',
              color: 'var(--brand-primary)', fontWeight: 600, fontSize: '0.875rem',
            }}
          >
...
export default function Navbar() {
  ...
  // ✅ Init immédiate depuis le cache — pas de flash client→prestataire
  const getInitialProviderState = () => {
    if (!user?.id) return false;
    const cached = getCachedProvider(user.id);
    if (cached !== null) return cached;
    if (user?.role === 'prestataire') return true;
    return false;
  };
  const [isProvider,           setIsProvider]           = useState(getInitialProviderState);
    ...
    try {
      ...
      if (newUnread.length > 0 && prevNotifsRef.current.length > 0) {
        ...
        notify({
          ...
          url:   data.url   || '/',
          showToast: false, playSound: true, showNative: true,
        });
      }
  ...
  const handleViewAll      = useCallback(() => { setShowNotifPanel(false); navigate('/notifications'); }, [navigate]);
  const handleToggleNotif  = useCallback(() => { setShowNotifPanel(p => !p); setShowSettingsDropdown(false); }, []);
  const handleCloseNotif   = useCallback(() => setShowNotifPanel(false), []);
  ...
  useEffect(() => {
    ...
    entrepriseApi.getMesEntreprises()
      .then(list => {
        ...
        const newIsProvider = validated.length > 0;
        setIsProvider(newIsProvider);
        setCachedProvider(user.id, newIsProvider);
        setHasPendingEntreprise(pending.length > 0 && !newIsProvider);
      })
  ...
  // ── Sections paramètres (vraies icônes partout) ───────────────────────────
  ...
  const UserAvatar = ({ size = 35 }) => {
    ...
    return (
      <div style={{
        ...
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        flexShrink: 0,
        border: ok ? '2px solid var(--brand-light)' : 'none',
      }}>
        {ok
  ...
  const bellProps = {
    unreadCount, showPanel: showNotifPanel,
    onToggle: handleToggleNotif, onClose: handleCloseNotif,
  ...
  return (
    <>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'nowrap' }} className="desktop-nav">
            {user ? (
              <>
                  ...
                  <>
                    ...
                    {hasPendingEntreprise && !isProvider && (
                      <Link to="/dashboard" className="nav-link" style={{
                        ...
                        border: '1px solid #fbbf24', fontWeight: 600,
                      }}>
                        <FiClock style={{ fontSize: '1rem' }} />
                        <span className="nav-label">Validation en cours</span>
                      </Link>
                    )}
                  </>
                )}
                ...
                <div ref={settingsRef} style={{ position: 'relative' }}>
                  ...
                  >
                    ...
                    <span className="user-name" style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: '0.9rem' }}>
                      {user.name.split(' ')[0]}
                    </span>
                    ...
                    <FiChevronDownIcon style={{
                      fontSize: '0.875rem', color: 'var(--text-secondary)',
                      transition: 'transform 0.3s',
                      transform: showSettingsDropdown ? 'rotate(180deg)' : 'rotate(0)',
                    }} />
                  </button>
                  ...
                  {showSettingsDropdown && (
                    <div className="settings-dropdown-menu" style={{
                      ...
                      backgroundColor: 'var(--bg-card)', borderRadius: '1rem',
                      boxShadow: 'var(--shadow-lg)', border: '2px solid var(--border-color)',
                      minWidth: 'min(380px, 85vw)', maxHeight: '70vh',
                      overflow: 'auto', zIndex: 1001,
                    }}>
                      {/* Header dropdown */}
                      <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                          <div style={{ flexShrink: 0, borderRadius: '50%', overflow: 'hidden', border: '3px solid var(--brand-primary)' }}>
                            <UserAvatar size={52} />
                          </div>
                          <div>
                            <h3 style={{ fontSize: '1.05rem', fontWeight: 700, color: 'var(--brand-primary)', marginBottom: '0.25rem' }}>
                              {user.name}
                            </h3>
                            ...
                            {isProvider && (
                              <span style={{ ...badgeStyle('#10b981'), display: 'inline-block', marginTop: '0.25rem' }}>
                                Prestataire
                              </span>
                            )}
                            {hasPendingEntreprise && !isProvider && (
                              <span style={{ ...badgeStyle('#f59e0b'), display: 'inline-flex', alignItems: 'center', gap: '0.25rem', marginTop: '0.25rem' }}>
                                <FiClock size={10} /> Validation en cours
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <div style={{ padding: '0.75rem' }}>
                        {settingsSections.map(({ id, label, icon: Icon, description, color }) => (
                          <button
                            key={id}
                            onClick={() => { navigate(`/settings?tab=${id}`); setShowSettingsDropdown(false); }}
                          ...
                          >
                            <div style={{
                              ...
                              backgroundColor: `${color}20`, color, fontSize: '1.125rem', flexShrink: 0,
                            }}>
                              <Icon />
                            </div>
                      ...
                      <div style={{ padding: '0.75rem 1.5rem 1.25rem', borderTop: '1px solid var(--border-color)' }}>
                        <Link
                          ...
                          style={{
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            gap: '0.5rem', width: '100%', padding: '0.75rem', marginBottom: '0.75rem',
                            backgroundColor: 'transparent', color: 'var(--brand-primary)',
                            border: '2px solid var(--brand-primary)', borderRadius: '0.5rem',
                            cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem', textDecoration: 'none',
                          }}
              ...
              <>
                ...
                <div style={{ position: 'relative' }} onMouseEnter={() => setShowServicesDropdown(true)} onMouseLeave={() => setShowServicesDropdown(false)}>
                  ...
                  {showServicesDropdown && (
                    <div className="mega-dropdown" style={{
                      ...
                      transform: 'translateX(-50%)', marginTop: '0.5rem',
                      backgroundColor: 'var(--bg-card)', borderRadius: '1rem',
                      boxShadow: 'var(--shadow-lg)', border: '2px solid var(--border-color)',
                      width: '90vw', maxWidth: '700px', maxHeight: '70vh', overflow: 'auto',
                    }}>
                ...
                <Link to="/login" style={{ backgroundColor: 'transparent', color: 'var(--brand-primary)', border: '2px solid var(--brand-primary)', padding: '0.625rem 1.5rem', borderRadius: '0.75rem', fontWeight: 600, textDecoration: 'none', fontSize: '0.9rem', transition: 'all 0.3s', whiteSpace: 'nowrap' }}>
                  Connexion
                </Link>
                <Link to="/register" style={{ backgroundColor: 'var(--brand-primary)', color: '#fff', border: 'none', padding: '0.75rem 1.5rem', borderRadius: '0.75rem', fontWeight: 600, textDecoration: 'none', fontSize: '0.9rem', boxShadow: 'var(--shadow-md)', transition: 'all 0.3s', whiteSpace: 'nowrap' }}>
                  Inscription
                </Link>
              </>
            )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <button
              ...
              style={{ display: 'none', backgroundColor: 'transparent', border: 'none', color: 'var(--brand-primary)', cursor: 'pointer', padding: '0.5rem', borderRadius: '0.5rem', flexShrink: 0 }}
            >
          ...
          <div onClick={e => e.stopPropagation()} style={{ backgroundColor: 'var(--bg-card)', width: '85%', maxWidth: 350, height: '100%', padding: '2rem 1.5rem', overflowY: 'auto', boxShadow: '-4px 0 20px rgba(0,0,0,0.3)', animation: 'slideInRight 0.3s ease' }}>
            ...
            {user && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', backgroundColor: 'var(--bg-secondary)', borderRadius: '0.75rem', marginBottom: '1.5rem', border: '1px solid var(--border-color)' }}>
                ...
                {unreadCount > 0 && (
                  <span style={{ backgroundColor: '#ef4444', color: '#fff', borderRadius: 999, minWidth: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.72rem', fontWeight: 800 }}>
                    {unreadCount > 99 ? '99+' : unreadCount}
                  </span>
                )}
              </div>
                ...
                <>
                  {user.role === 'admin' ? (
                    ...
                    <>
                      ...
                      {hasPendingEntreprise && !isProvider && (
                        <MobileLink to="/dashboard" icon={FiClock} label="Suivi de validation" onClick={() => setMobileMenuOpen(false)} />
                      )}
                    </>
                  )}
                  ...
                  {/* ✅ Notifications dans le menu mobile avec vraie icône */}
                  <div
                  ...
                  >
                    ...
                    {unreadCount > 0 && (
                      <span style={{ marginLeft: 'auto', backgroundColor: '#ef4444', color: '#fff', borderRadius: 999, minWidth: 20, height: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', fontWeight: 800 }}>
                        {unreadCount}
                      </span>
                    )}
                  </div>
                  ...
                  <button
                    onClick={handleLogout}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', width: '100%', padding: '1rem', marginTop: '1rem', backgroundColor: 'var(--brand-primary)', color: '#fff', border: 'none', borderRadius: '0.75rem', cursor: 'pointer', fontWeight: 600, fontSize: '1rem' }}
                  >
                    <FaSignOutAlt /> {t('nav.logout')}
                  </button>
                </>
      ...
      <style>{`
        ...
        .nav-link { text-decoration: none; padding: 0.5rem 0.75rem; display: flex; align-items: center; gap: 0.5rem; white-space: nowrap; border-radius: 0.5rem; font-size: 0.9rem; position: relative; transition: all 0.2s ease; }
